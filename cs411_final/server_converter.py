from convert import convert2mp4
import sys
...
PORT = 7766 # must be input parameter @TODO

def handler(clientsock, addr):
	data = clientsock.recv(BUFF)
	print repr(addr) + ' recv:' + repr(data)
	input_data = str.split(data);
	intput_filename = input_data[0]
	output_filename = input_data[1]
	convert2mp4(intput_filename, output_filename)
	clientsock.close()
