...
(function() {
  ...
  methods = {
	...
	loadRows: function(rows) {
      ...
      rows = $(rows);
	  //node = this.data("treetable").tree[node];
	  ...
	  */
	  rows.each(function() {
			node = tree[$(this).data(settings.parentIdAttr)];
			if (node == null) { // Inserting new root nodes
				treeTable.append($(this));
			} else if (node.children.length > 0) {
				$(this).insertAfter(node.children[node.children.length-1].row);
			} else {
				$(this).insertAfter(node.row);
			}
      });
    ...
    unloadBranch: function(node) {
		var tree = this.data("treetable").tree;
      this.data("treetable").unloadBranch(tree[node]);
      return this;
    },
		
	unloadNode: function(node) {
		var tree = this.data("treetable").tree,
			nodes = this.data("treetable").nodes;
		this.data("treetable").unloadBranch(tree[node]);
		console.log(tree[node].parentNode());
		tree[node].row.remove();

		// Clean up Tree object (so Node objects are GC-ed)
		delete tree[tree[node].id];
		nodes.splice($.inArray(tree[node], nodes), 1);

		return this;
	},
	clear : function()
	{
		 var $this = this.data("treetable");
