...
(function($) {
	...
	$.fn.fileUploader = function(config){
		...
		config = $.extend({}, {
			...
			buttonStartOver: '#px-startOver',
			buttonView: '#px-view',
			selectFileLabel: 'Select files',
			allowedExtension: 'mp4|rmvb|mkv|avi|m4v|mpg|mov|wmv|3gp|mpeg|mpe|ogm|flv|divx',
		...
		var px = {
			...
			init: function(){
				...
				px.form.find(config.buttonUpload + ',' + config.buttonClear + ',' + config.buttonView).appendTo(pxButton);
		...
		*/
		function afterEachUpload(formId, data, status, errorThrown) {
			...
			if (status == 'success'){
				...
				$uploadData.children('.status').html( $(data).find('#message').load() );
				
				$(config.buttonView).show();
		...
		*/
		...
		$(config.buttonView, pxButton).click(function(){
			
			window.location.href = "test_step3.php";
		});
