...
void normalizeColors(MyImage * myImage){
	...
	for(int i=0;i<NSAMPLES;i++){
		//cout << "sample point " << i << "\n";
		//cout << avgColor[i][0] << " " << avgColor[i][1] << " " << avgColor[i][2] << "\n";
...
double showWindows(MyImage m){
	...
	//Size s = m.bw.size();
	double skin_area = 0;
	for (int i=0; i < m.bw.rows; i++){
		for (int j=0; j<m.bw.cols; j++){

			if (m.bw.at<double>(i,j) != 0)
				skin_area = skin_area+1;
				//cout <<m.bw.at<double>(i,j)<<" ";
		}
	}
	//cout << "\n";
	double skin_ratio = skin_area/m.bw.rows/m.bw.cols;
	//cout << skin_ratio << "\n";
	return skin_ratio;
	//cout << "m.bw.rows" << m.bw.rows << "\n";
	//cout << "m.bw.cols" << m.bw.cols << "\n";
	//imshow("img1",m.bw);	
}
...
int main(int argc, char *argv[]){
	MyImage m(argv[1]);
	//MyImage m("/Applications/MAMP/htdocs/eulerian-magnification/handDetectionCV/parkinson2_2.mp4");		
	HandGesture hg;
	init(&m);		
	m.cap >>m.src;
 //    namedWindow("img1",CV_WINDOW_KEEPRATIO);
	...
	// average(&m);
	// destroyWindow("img1");
	avgColor[0][0] = 9;  avgColor[0][1] = 140; avgColor[0][2] = 78;
	avgColor[1][0] = 12; avgColor[1][1] = 125; avgColor[1][2] = 126;
	avgColor[2][0] = 12; avgColor[2][1] = 182; avgColor[2][2] = 160;
	avgColor[3][0] = 7;  avgColor[3][1] = 142; avgColor[3][2] = 63;
	avgColor[4][0] = 11; avgColor[4][1] = 152; avgColor[4][2] = 97;
	avgColor[5][0] = 8;  avgColor[5][1] = 142; avgColor[5][2] = 56;
	avgColor[6][0] = 11; avgColor[6][1] = 148; avgColor[6][2] = 90;
